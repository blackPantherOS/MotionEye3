...
def find_ffmpeg():
    ...
    # version
    try:
        # vector fix me: output = subprocess.check_output(binary + ' -version', shell=True)
        output = subprocess.check_output(binary + str(' -version').encode('utf-8'), shell=True)
    ...
    # codecs
    try:
        # vector fix me: output = subprocess.check_output(binary + ' -codecs -hide_banner', shell=True)
        output = subprocess.check_output(binary + str(' -codecs -hide_banner').encode('utf-8'), shell=True)
        output = output.decode('utf-8')
    ...
    # vector fix me restore encoded output
...
def get_media_content(camera_config, path, media_type):
    ...
    try:
        ...
        logging.error('1failed to read file %(path)s: %(msg)s' % {
                'path': full_path, 'msg': str(e)})
...
def get_media_preview(camera_config, path, media_type, width, height):
    ...
    try:
        logging.debug('try read thumb file on %(path)s path' % {
                'path': full_path})
        with open(full_path,'rb') as f:
            content = f.read()

    except Exception as e:
        logging.error('failed to read thumb file %(path)s: %(msg)s' % {
                'path': full_path, 'msg': str(e)})
    ...
    #vector fix me, not a string: sio = io.StringIO()

    try:
        logging.debug('try load preview image ...')
        #sio = io.BytesIO(content)
        #image = Image.open(sio)
        image = Image.open(io.BytesIO(content))
        logging.debug('loaded preview image ...')
    ...
    #vector fix me, not a string: sio = StringIO()
