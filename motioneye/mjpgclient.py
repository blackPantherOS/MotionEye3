...
class MjpgClient(IOStream):
    ...
    def _seek_http(self):
        ...
        self.read_until_regex('HTTP/1.\d \d+ '.encode("utf-8"), self._on_http)

    def _on_http(self, data):
        if data.endswith('401 '.encode("utf-8")):
    ...
    def _seek_content_length(self):
        ...
        self.read_until('Content-Length:'.encode("utf-8"), self._on_before_content_length)

    def _on_before_content_length(self, data):
        if self._check_error():
            return

        self.read_until('\r\n\r\n'.encode("utf-8"), self._on_content_length)

    def _on_content_length(self, data):
        if self._check_error():
            return

        matches = re.findall('(\d+)'.encode("utf-8"), data)
        if not matches:
